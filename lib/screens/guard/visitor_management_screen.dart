...
class GuardVisitorManagementScreen extends StatefulWidget {
  ...
  @override
  State<GuardVisitorManagementScreen> createState() => _VisitorManagementScreenState();
}

class _VisitorManagementScreenState extends State<GuardVisitorManagementScreen> {
  ...
  final List<Map<String, dynamic>> _quickActions = [
    {'icon': Icons.qr_code_scanner, 'label': 'QR Scanner', 'color': Colors.purple, 'screen': 'qr_scanner'},
    {'icon': Icons.group, 'label': 'Group Entry', 'color': Colors.blue, 'screen': 'group_entry'},
    {'icon': Icons.build, 'label': 'Vendor Access', 'color': Colors.green, 'screen': 'vendor_access'},
    {'icon': Icons.directions_car, 'label': 'Vehicle', 'color': Colors.orange, 'screen': 'vehicle_tracking'},
  ];

  // Map tab index to API status filter
  String? get _statusFilter {
    const map = {1: 'pending', 2: 'approved', 3: 'rejected', 4: 'entered', 5: 'exited'};
  ...
  void _loadVisitors() {
    context.read<GuardBloc>().add(LoadVisitors(status: _statusFilter, limit: 50));
  }

  void _onTabChanged(int index) {
    setState(() => _selectedIndex = index);
    context.read<GuardBloc>().add(LoadVisitors(
      status: (() {
        const map = {1: 'pending', 2: 'approved', 3: 'rejected', 4: 'entered', 5: 'exited'};
        return map[index];
      })(),
  ...
  void _showPermissionDialog() {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content: const Text('Camera access is required to scan QR codes. Please grant permission in settings.'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context), child: const Text('Cancel')),
          ElevatedButton(onPressed: () { Navigator.pop(context); openAppSettings(); }, child: const Text('Settings')),
        ],
      ),
    );
  }
        ...
        content: Column(
          ...
          children: [
            ...
            ListTile(
              ...
              onTap: () { Navigator.pop(context); _showAddVisitorForm(); },
            ),
          ],
        ),
        ...
        listener: (ctx, state) {
          if (state is VisitorAdded) {
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('Visitor added successfully'), backgroundColor: AppTheme.success),
            );
            _loadVisitors();
          ...
          child: Form(
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  Text('Add Visitor', style: TextStyle(fontSize: 20.sp, fontWeight: FontWeight.bold)),
                  ...
                  TextFormField(
                    ...
                    decoration: const InputDecoration(labelText: 'Resident ID *', border: OutlineInputBorder(), hintText: 'Enter resident ID'),
                    keyboardType: TextInputType.number,
                    validator: (v) => (v == null || v.isEmpty) ? 'Resident ID is required' : null,
                  ),
                  SizedBox(height: 12.h),
                  StatefulBuilder(
                    builder: (ctx, setStateSB) => DropdownButtonFormField<String>(
                      initialValue: visitorType,
                      decoration: const InputDecoration(labelText: 'Visitor Type', border: OutlineInputBorder()),
                      items: const [
                      ...
                      onChanged: (v) => setStateSB(() => visitorType = v ?? 'guest'),
                    ),
                  ),
                  ...
                  BlocBuilder<GuardBloc, GuardState>(
                    builder: (ctx, state) {
                      return ElevatedButton(
                        onPressed: state is GuardLoading ? null : () {
                          if (formKey.currentState!.validate()) {
                            ctx.read<GuardBloc>().add(AddVisitor(
                              name: nameCtrl.text.trim(),
                              phone: phoneCtrl.text.trim(),
                              purpose: purposeCtrl.text.trim(),
                              visitorType: visitorType,
                              residentId: int.tryParse(residentIdCtrl.text.trim()),
                            ));
                          }
                        },
                        ...
                        child: state is GuardLoading
                            ...
                            : const Text('Add Visitor', style: TextStyle(color: Colors.white)),
                      );
                    },
                  ),
                ],
                              ...
                              children: [
                                GestureDetector(
                                  ...
                                  child: Container(
                                    ...
                                    child: Icon(action['icon'], color: Colors.white, size: 30.sp),
                                  ),
                                ),
                                SizedBox(height: 8.h),
                                Text(action['label'], textAlign: TextAlign.center, style: const TextStyle(fontSize: 12)),
                              ],
                            );
                          },
                        ),
                          ...
                          child: ListView.builder(
                            ...
                            itemBuilder: (context, index) => Padding(
                              ...
                              child: ChoiceChip(
                                ...
                                onSelected: (selected) { if (selected) _onTabChanged(index); },
                              ),
                            ),
                          ),
                        ),
                                  ...
                                  ? Center(
                                      child: Column(
                                        ...
                                        children: [
                                          ...
                                          SizedBox(height: 16.h),
                                          Text('No visitors found', style: TextStyle(color: Colors.grey, fontSize: 16.sp)),
                                        ],
                                      ),
                                    )
                                  : ListView.builder(
  ...
  Widget _buildVisitorCard(Map<String, dynamic> visitor) {
    ...
    switch (status) {
      ...
      case 'entered': statusColor = AppTheme.primary; break;
      case 'exited': statusColor = Colors.grey; break;
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(visitor['name'] ?? '', style: TextStyle(fontSize: 16.sp, fontWeight: FontWeight.bold)),
                      SizedBox(height: 4.h),
                      Text(visitorType.toUpperCase(), style: TextStyle(fontSize: 13.sp, color: Colors.grey)),
                      if (residentName.isNotEmpty)
                        Text('For: $residentName', style: TextStyle(fontSize: 12.sp, color: Colors.grey), overflow: TextOverflow.ellipsis),
                      if (visitor['phone'] != null)
                        Text('${visitor['phone']}', style: TextStyle(fontSize: 12.sp, color: Colors.grey)),
                      SizedBox(height: 6.h),
                      Container(
                        ...
                        child: Text(status.toUpperCase(), style: TextStyle(color: statusColor, fontSize: 11.sp, fontWeight: FontWeight.bold)),
                      ),
                    ],
                  ),
                ),
            ...
            if (status == 'pending') ...[
              ...
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  ...
                  label: const Text('Mark Entered', style: TextStyle(color: Colors.white)),
                ),
              ),
              ...
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  ...
                  label: const Text('Mark Exited', style: TextStyle(color: Colors.white)),
                ),
              ),
            ],
          ],
